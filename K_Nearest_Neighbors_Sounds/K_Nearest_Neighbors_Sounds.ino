...
String Object_Types[NUM_OF_OBJECT_TYPES] = {"Dog Barking", "Car Alarm", "Door Bell"};
...
// FIXME: Not the most accurate form of comparison.
// May be good for now.
// Compares the frequency arrays from the currObject and the knownObject
// Returns the average difference between the arrays
float CompareFrequencyArray(Object currObject, Object knownObject, CompareObject &compObject) {
  int array_size = min(currObject.freq_size, knownObject.freq_size);
  // The compObject takes the difference between the currObject and the knownObject
  float avg_dist;
  float sum = 0;
  for(int i = 0; i < array_size; ++i) {
    compObject.freq[i] = abs(currObject[i] - knownObject[i]);
    sum += compObject.freq[i];
  }
  avg_dist = sum / array_size;
  return avg_dist;
}
...
String ObjectPatternRecognition(Object currObject, Object knownObjects[]) {
  ...
  CompareObject compObj;
  
  Serial.print("Object frequency: ");
  
  for(int i = 0; i < currObject.freq_size; ++i) {
    Serial.println(currObject.freq[i]);
  }
 
  // First K known objects are the closest K, obviously, so just fill array
  for(int i = 0; i < K; ++i) {
    kNearestObjects[i].type = knownObjects[i].type;
    // Fill the frequency array for the closest K objects.
    for(int j = 0; j < freq_size; ++j) {
      kNearestObjects[i].freq[j] = knownObjects[i].freq[j];
    }
  }
  ...
  for(int i = K; i < NUM_OF_KNOWN_OBJECTS; ++i) { // For each remaining known object
    ...
    // FIXME: Figure out comparison from compare frequency array and average
    ...
    for(j = 0; j < K; ++j) {
      ...
      temp_dist = CompareFrequencyArray(currObject, kNearestObjects[j], compObj);
    ...
    if(temp_dist < max_diff_of_K_nearest) {
      ...
      // Replace the frequency array as well
      for(int j = 0; j < kNearestObjects[max_type].freq_size; ++j) {
        kNearestObjects[max_index].freq[j] = knownObjects[i].freq[j];
      }
  ...
  for(int i = 0; i < K; ++i) {
    for(int j = 0; j < currObject.freq_size; ++j) {
      Serial.println(currObject[i].freq[j]);
    }
  }
...
void loop() {
  ...
  if (tFrequency > SIGNAL_MAX_ERROR_CODE) {
    ...
    // Plausability check, don't output anything if error
    tFrequency = doPlausi();
    if (tFrequency == SIGNAL_FIRST_LAST_PLAUSI_FAILED) {
      ...
      */
      ...
      // FIXME: This should be in feature extraction
      computeDirectAndFilteredMatch(tFrequency);
      if (ToneDetectorControl.ToneMatchDirect == TONE_MATCH && !wait) {
        // signal match
        digitalWrite(LED_BUILTIN, HIGH);
        
        // Do sound detection thing
      ...
      else if(ToneDetectorControl.ToneMatchDirect == TONE_MATCH && wait) {
        digitalWrite(LED_BUILTIN, LOW);
      }
    }
  }
